...
function MoviesAdminPage() {
    ...
    const handleDeleteSelectedMovies = async () => {
        const confirmed = window.confirm("Are you sure you want to delete selected movies?");
        if (confirmed) {
            try {
                await Promise.all(selectedMovies.map(movieId => MovieService.deleteMovie(movieId)));
                setMovies(prevMovies => prevMovies.filter(movie => !selectedMovies.includes(movie.id)));
                setSelectedMovies([]);
            } catch (error) {
    ...
    const handleCheckboxChange = (event: React.ChangeEvent<HTMLInputElement>, movieId: string) => {
        if (event.target.checked) {
            setSelectedMovies(prevSelected => [...prevSelected, movieId]);
        } else {
            setSelectedMovies(prevSelected => prevSelected.filter(id => id !== movieId));
        }
    };
    ...
    return (
        <div className="movies-admin-page">
            <div>
                ...
                <button className="select-movie-button button" onClick={() => setShowCheckboxes(!showCheckboxes)}>
                    {showCheckboxes ? "Hide Checkboxes" : "Select Movies"}
                </button>
                <button className="create-movie-button button" onClick={() => navigate("/movie/create")}>Create New Movie</button>

                {showCheckboxes && selectedMovies.length > 0 && (
                    <button className="delete-selected-movie-button button" onClick={handleDeleteSelectedMovies}>
                    ...
                    <tbody>
                        {movies.map((movie, index) => (
                            <tr key={movie.id}>
                                ...
                                <td>
                                    <div className="enlarged-image" style={{ display: fullscreenImageUrl ? 'flex' : 'none' }} onClick={() => setFullscreenImageUrl(null)}>
                                        <img src={fullscreenImageUrl || ''} alt="Fullscreen Movie" className="fullscreen-image" />
                                    </div>
                                    <img
                                        src={movie.imageLink}
                                        alt={movie.title}
