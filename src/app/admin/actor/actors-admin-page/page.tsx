...
function ActorsPage() {
    ...
    const handleActorDetails = (actorId: string) => {
        router.push(`/actor-pages/actor/${actorId}`);
    };
    ...
    const handleDeleteSelectedActors = async () => {
        const confirmed = window.confirm(
            "Are you sure you want to delete selected actors?"
        );
        if (confirmed) {
            try {
                await Promise.all(
                    selectedActors.map((actorId) => ActorService.deleteActor(actorId))
                );
                setActors((prevActors) =>
                    prevActors.filter((actor) => !selectedActors.includes(actor.id))
                );
                setSelectedActors([]);
            } catch (error) {
    ...
    return (
        <div className="actors-admin-page">
            <div>
                ...
                <button
                    className="select-actor-button button"
                    onClick={() => setShowCheckboxes(!showCheckboxes)}
                >
                    {showCheckboxes ? "Hide Checkboxes" : "Select Actors"}
                </button>
                <button
                    className="create-actor-button button"
                    onClick={() => router.push("/actor-pages/create/ActorFormPage")}
                >
                    Create New Actor
                </button>
                ...
                {showCheckboxes && selectedActors.length > 0 && (
                    <button
                        className="delete-selected-actor-button button"
                            ...
                            <tr key={actor.id}>
                                ...
                                <td>
                                    <div
                                        ...
                                        style={{
                                            display:
                                                hoveredImageUrl === actor.image ? "block" : "none",
                                        }}
                                    >
